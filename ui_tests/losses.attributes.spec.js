...
test.describe("Losses Page Attributes Verification", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.screenshot({ path: "ui_tests/pic_generated_in_tests/losses-page.png" });
  });

  test("Losses page title is visible", async ({ page }) => {
  ...
  test("Losses count indicator is present", async ({ page }) => {
  ...
  test("Losses search field is functional", async ({ page }) => {
    await page.screenshot({ path: 'ui_tests/pic_generated_in_tests/losses-search-field.png' });
  ...
  test("Losses filter options are visible", async ({ page }) => {
  ...
  test("Losses table or list is visible", async ({ page }) => {
    await page.screenshot({ path: 'ui_tests/pic_generated_in_tests/losses-table.png' });
    ...
    if (tableFound) {
      ...
      console.log("No table/list structure found, checking for any loss items");
      ...
      try {
        ...
        if (await dataContent.isVisible()) {
          ...
          const bodyText = await page.textContent('body');
          expect(bodyText).toContain('Убытки');
          console.log("No structured data view found, but page contains 'Убытки'");
        }
      } catch (e) {
  ...
  test("Loss item contains key information", async ({ page }) => {
    await page.screenshot({ path: 'ui_tests/pic_generated_in_tests/loss-items.png' });
    
    const tableRows = page.locator('table tr, .v-data-table__tr');
    const rowCount = await tableRows.count();
    
    if (rowCount > 0) {
      console.log(`Found ${rowCount} table rows that may contain loss information`);
      ...
      console.log("No table rows found, checking for any loss-related content");
      
      const pageContent = await page.textContent("body");
      const hasLossContent = pageContent.includes('Убытки') || 
  ...
  test("Interactive elements are present on loss items", async ({ page }) => {
    await page.screenshot({ path: 'ui_tests/pic_generated_in_tests/loss-items-interactivity.png' });
    ...
    if (lossItem) {
      ...
      if (interactiveCount > 0) {
        ...
        console.log("No interactive elements found, but page has loss content");
      }
    } else {
  ...
  test("Page contains action button for adding losses", async ({ page }) => {
